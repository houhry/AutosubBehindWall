...
from speechrecognize import speech_recognizai_baidu, speech_recognizai_ali, speech_recognizai_tc, speech_recognizai_xf, speech_recognizai_azure
from speechrecognize import init_baidu, init_ali, init_tc, init_xf
...
if __name__ =="__main__":
    wavepath=r'C:\Users\Harley\PycharmProjects\sub\sub.wav'
  ...
  #  laguage='en'
    ...
    for i in range(len(splittimestamp)-1):
        ...
        if framerate==8000 or framerate==16000:
            ...
            #if response['err_no']==0: #baidu
            #   recognize_txt=(response['result'][0])
            # if response['status'] == 20000000: #ali
            #   recognize_txt=(response['result'][0])
            if RECOGNITION_METHOD == "BAIDU":
                init_baidu()
                response = speech_recognizai_baidu(wav[starttime * 1000:endtime * 1000].raw_data, framerate, laguage)
                if response['err_no']==0: #baidu
                    recognize_txt=(response['result'][0])
            elif RECOGNITION_METHOD == "TENCENT":
                init_tc()
                response = speech_recognizai_tc(wav[starttime * 1000:endtime * 1000].raw_data, framerate, laguage)
                if True:
                    recognize_txt=(response['data']['text'])
            elif RECOGNITION_METHOD == "ALI":
                init_ali()
                response = speech_recognizai_ali(wav[starttime * 1000:endtime * 1000].raw_data, framerate, laguage)
                if response['status'] == 20000000:  # ali
                    recognize_txt=(response['result'])
            elif RECOGNITION_METHOD == "XUNFEI":
                init_xf()
                response = speech_recognizai_xf(wav[starttime * 1000:endtime * 1000].raw_data, framerate, laguage)
                if True:
                    recognize_txt = (response['data'])
            elif RECOGNITION_METHOD == "AZURE":

                response = speech_recognizai_azure(wav[starttime * 1000:endtime * 1000].raw_data, framerate, laguage)
                if True:
                    recognize_txt = response
